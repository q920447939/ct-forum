package cn.withmes.forum.api.captcha.utils;



import cn.withmes.forum.api.captcha.Captcha;
import cn.withmes.forum.api.captcha.GifCaptcha;
import cn.withmes.forum.api.captcha.SpecCaptcha;
...
public class CaptchaUtil {
    ...
    private static String outCaptcha(int width, int height, int len, Font font, int cType, HttpServletRequest request, HttpServletResponse response)
        ...
        if (font != null && null != captcha) {
            captcha.setFont(font);
        }
        if (null != captcha && null != captcha.text()) {
            String text = captcha.text();
            request.getSession().setAttribute(SESSION_KEY, text.toLowerCase());
            captcha.out(response.getOutputStream());
            return captcha.text().replace("\"", "");
        }
        return "";
    }
